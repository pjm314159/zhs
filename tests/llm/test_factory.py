...
class TestLLMProviderFactoryCreate:
    ...
    def test_use_builtin_ai_returns_zhidao_provider(self) -> None:
        """use_builtin_ai=True 返回 ZhidaoAIProvider"""
        config = AIConfig(enabled=True, use_builtin_ai=True)
        session = MagicMock()
    ...
    def test_openai_api_key_returns_openai_provider(self) -> None:
        """有 api_key 且 use_builtin_ai=False 返回 OpenAIProvider"""
        config = AIConfig(
            enabled=True,
    ...
    def test_no_api_key_returns_none(self) -> None:
        ...
        config = AIConfig(enabled=True, use_builtin_ai=False, api_key="")
        assert LLMProviderFactory.create(config) is None

    def test_returns_llm_provider_subclass(self) -> None:
        """返回值是 LLMProvider 子类"""
        config = AIConfig(enabled=True, use_builtin_ai=False, api_key="sk-test")
        provider = LLMProviderFactory.create(config)
