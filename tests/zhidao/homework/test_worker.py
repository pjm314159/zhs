...
class TestHomeworkWorkerGenerateAnswer:
    """答案生成测试"""

    def test_generate_answer_from_cache_correct(self) -> None:
        """缓存有正确选项时直接使用"""
        ...
        # 设置缓存正确选项
        cache.mark_correct(100, "exam1", "abc123==", [102])

        answer = worker._generate_answer(question, item)
        assert answer == 102

    def test_generate_answer_from_cache_wrong_exclude(self) -> None:
        """缓存有错误选项时排除后随机选择"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
        ...
        # 标记 101 和 102 为错误
        cache.mark_wrong(100, "exam1", "abc123==", [101, 102])

        answer = worker._generate_answer(question, item)
        # 应从 103 或 104 中选择
        assert answer in (103, 104)

    def test_generate_answer_multi_choice_from_cache(self) -> None:
        """多选题缓存正确选项返回逗号分隔"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
        ...
        item = _make_item()
        question = _make_question(question_type_id=2)  # 多选题

        cache.mark_correct(100, "exam1", "abc123==", [101, 103])

        answer = worker._generate_answer(question, item)
        assert answer == "101,103"

    def test_generate_answer_no_cache_no_llm_random(self) -> None:
        """无缓存无 LLM 时随机选择"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
    ...
    def test_generate_answer_fill_blank_no_llm_returns_none(self) -> None:
        """填空题无 LLM 返回 None"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
    ...
    def test_generate_answer_no_eid_no_id(self) -> None:
        """题目无 eid 也无 id 返回 None"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
        worker = HomeworkWorker(session, config, cache)
        ...
        item = _make_item()
        question = _make_question(eid=None, qid=None)

        answer = worker._generate_answer(question, item)
        assert answer is None

    def test_generate_answer_with_llm_single_choice(self) -> None:
        """LLM 单选题生成"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
        ...
        llm = MagicMock()
        llm.single_choice.return_value = [103]
    ...
    def test_generate_answer_with_llm_multi_choice(self) -> None:
        """LLM 多选题生成"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
        ...
        llm = MagicMock()
        llm.multiple_choice.return_value = [101, 103]
    ...
    def test_generate_answer_with_llm_fill_blank(self) -> None:
        """LLM 填空题生成"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
        ...
        llm = MagicMock()
        llm.fill_blank.return_value = ["答案1", "答案2"]
        ...
        answer = worker._generate_answer(question, item)
        assert answer == "答案1,答案2"

    def test_generate_answer_llm_fails_fallback_random(self) -> None:
        """LLM 失败时回退到随机"""
        session = _make_mock_session()
        config = _make_config()
        cache = _make_cache()
        ...
        llm = MagicMock()
        llm.single_choice.side_effect = Exception("LLM error")
