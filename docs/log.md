...
### Task 34 — 发布流程收尾补强（发布后快进 dev、同步 uv.lock）✅
- 背景（v0.1.4.1 实跑暴露的两处缺口）
  - 方案 B 的发布提交（`pyproject.toml` 版本 + `CHANGELOG.md` 段落）只落在 `main`，发布后 `dev` 落后一个提交；若不处理，下一次 Prepare Release 从 `dev` 取文件会**丢失本次 CHANGELOG 段落**（`[0.1.5]` 之后直接跳到 `[0.1.4]`）
  - 发布只 bump `pyproject.toml`，`uv.lock` 中记录的本包版本未同步（实测发布后本地 `uv run` 会把 `uv.lock` 改脏：`0.1.4` → `0.1.4.1`）
- Green
  - `.github/workflows/release.yml`：PyPI 发布后新增 `Sync dev with main (fast-forward, best effort)` —— `git fetch origin main dev` + `git push origin origin/main:dev`（纯快进，`dev` 已有新提交时会失败而非覆盖；`continue-on-error: true`，失败不影响发布结果）
  - `.github/workflows/prepare-release.yml`：`Bump version (pyproject.toml + uv.lock)` 用 `python3` 正则同步 `uv.lock` 中 `[[package]] name = "zhs"` 的版本（找不到条目则报错退出）；提交时一并 `git add uv.lock`
  - `docs/linter.md` §9.3 / §9.4：流程补上"末尾快进同步 dev"与"版本号同步含 uv.lock"，并注明仓库设置 *Allow GitHub Actions to create and approve pull requests* 已开启
